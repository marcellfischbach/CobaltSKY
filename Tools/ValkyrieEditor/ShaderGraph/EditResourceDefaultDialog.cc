...
void EditResourceDefaultDialog::SetNode(SGNode *node)
{
  ...
  m_gui.leTextureRef->setText(QString(node->GetDefaultTexture().GetResourceFile().c_str()));
...
void EditResourceDefaultDialog::on_pbOK_clicked(bool)
{
  ...
  case eSPT_Matrix4:
    ...
    break;
  case eSPT_Texture:
    texture = vkResourceLocator(vkString((const char*)m_gui.leTextureRef->text().toLatin1()));
    break;
  }
