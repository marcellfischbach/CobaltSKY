...
void main ()
{
	...
	// Vertex 0
	gl_Position = cs_MatProj * (cs_MatView * gl_in[0].gl_Position + rot * vec4(minS.x, 0.0f, minS.y, 0.0));
	inFragTexCoord = vec2 (0, 0);
	EmitVertex ();

	// Vertex 1
	gl_Position = cs_MatProj * (cs_MatView * gl_in[0].gl_Position + rot * vec4(minS.x, 0.0f, maxS.y, 0.0));
	inFragTexCoord = vec2 (0, 1);
	EmitVertex ();
	
	
	// Vertex 2
	gl_Position = cs_MatProj * (cs_MatView * gl_in[0].gl_Position + rot * vec4(maxS.x, 0.0f, minS.y, 0.0));
	inFragTexCoord = vec2 (1, 0);
	EmitVertex ();
	
	// Vertex 3
	gl_Position = cs_MatProj * (cs_MatView * gl_in[0].gl_Position + rot * vec4(maxS.x, 0.0f, maxS.y, 0.0));
